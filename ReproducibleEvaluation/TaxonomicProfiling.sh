...
for filter in grim_original_tweak magnet hd adjacency-filter base-counting qgram shd shouji sneakysnake grim_original edlib
  do
    ...
    /usr/bin/time -v --output "$out"/Timing/profiling-time__"$filter"_n"$n"_r"$r".txt python3 $DIR/Scripts/read_mapping.py $sam/"$filter"__n"$n"_r"$r".sam ../Data/RefData/test/ --input_type sam --verbose --output "$out"/Profiles/"$filter"__n"$n"_r"$r".tsv
    echo "================================================ \n\n"
  done
